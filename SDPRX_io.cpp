...
void read_bed(gsl_matrix *snp, const string &bed_path, \
	const size_t n_samples, size_t left, size_t right, const std::vector<size_t>& idxi) 
{
    FILE *bed_fp;
	...
	std::vector<size_t> idxio = idxi;
	std::sort(idxio.begin(), idxio.end());
    ...
    for (size_t snp_idx = 0; snp_idx < n_snps; snp_idx++) 
	{
		...
		if (snp_idx != idxio[idxidx] - left) continue;

		for (size_t i = 0; i < n_samples; i++) 
		{
	    	gsl_matrix_set(snp, idxidx, i, geno[i]);
		}
		idxidx ++;
    }
